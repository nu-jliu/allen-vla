#!/bin/bash
# Inference Server Script for PI0 Policy
# Runs the model on a GPU machine and serves predictions over HTTP
# Use with inference_pi0_client.bash on the robot machine
...
print_banner() {
    echo -e "${CYAN}"
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║          PI0 Policy - Inference Server Script             ║"
...
print_usage() {
    ...
    echo "  $0 --checkpoint myuser/pi0-so101-pick_cube --port 8080"
    ...
    echo "  ./inference_pi0_client.bash --server-host <this-machine-ip>"
}
...
main() {
    ...
    cd "${PROJECT_ROOT}"
    exec uv run policy/pi0/inference_server.py \
