#!/bin/bash
# Inference Server Script for ACT Policy
# Runs the model on a GPU machine and serves predictions over HTTP
# Use with inference_act_client.bash on the robot machine
...
print_banner() {
    echo -e "${CYAN}"
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║          ACT Policy - Inference Server Script             ║"
...
print_usage() {
    ...
    echo "  CHECKPOINT          Model checkpoint from HuggingFace Hub"
    ...
    echo "  CHECKPOINT=myuser/act-so101-pick_cube PORT=8080 $0"
    echo ""
    echo -e "${BLUE}Client Connection:${NC}"
    echo "  After starting this server, run the client on the robot machine:"
    echo "  SERVER_HOST=<this-machine-ip> ./inference_act_client.bash"
}
