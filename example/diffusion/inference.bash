#!/bin/bash
# Inference Script for Diffusion Policy (Standalone)
# Evaluation dataset will be pushed to: {username}/eval_{policy}-{robot}-{task}
# Example: jliu6718/eval_diffusion-so101-place_brick
...
print_banner() {
    echo -e "${CYAN}"
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║     Diffusion Policy - Standalone Inference Script        ║"
    echo "╚═══════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
print_usage() {
    ...
    echo "  --episode N        Number of episodes to run (default: 10)"
...
print_config() {
    ...
    echo -e "  ${YELLOW}Eval Repo:${NC}       ${USERNAME}/eval_diffusion-${ROBOT_TYPE}-${TASK}"
...
main() {
    ...
    cd "${PROJECT_ROOT}"
    exec uv run policy/diffusion/inference.py \
