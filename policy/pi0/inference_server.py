#!/usr/bin/env python
"""
Inference Server for PI0 policy.

This server loads a trained PI0 policy and handles inference requests over TCP.
...
class InferenceServer:
    ...
    def load_policy(self) -> None:
        ...
        # Load policy config (following inference pattern)
    ...
    def _load_policy_fallback(self, policy_cfg: PreTrainedConfig) -> None:
        ...
        logger.info("Loading PI0 policy directly from checkpoint...")
        self.policy = PI0Policy.from_pretrained(self.checkpoint)
    ...
    def start(self, host: str = "0.0.0.0", port: int = 8000) -> None:
        ...
        """
        logger.info("=" * 60)
        logger.info("PI0 Policy Inference Server")
        logger.info("=" * 60)
        logger.info("")
...
def parse_args() -> Namespace:
    ...
    parser = ArgumentParser(
        description="PI0 Policy Inference Server",
        formatter_class=RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Start server with local checkpoint
  python policy/pi0/inference_server.py \\
    --checkpoint ./outputs/pi0_training/pretrained_model \\
    --port 8000

  # Start server with HuggingFace model (format: username/policy-robot-task)
  python policy/pi0/inference_server.py \\
    --checkpoint username/pi0-so101-place_brick \\
    --device cuda \\
    --port 8000
        """,
    )
